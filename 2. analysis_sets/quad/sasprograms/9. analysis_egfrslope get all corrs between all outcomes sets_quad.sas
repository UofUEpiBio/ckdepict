...
libname Results  "&path1\analysis_sets\quad\correlations\results";
...
data t1;
 ...
 if compress(label) in ("beta21-beta22")                   then var="quad   ";
 if compress(label) in ("beta21")                          then var="quad1  ";
 if compress(label) in ("beta22")                          then var="quad2  ";
...
data allcorr;
 ...
 drop ev1_ev1_corr ev2_ev2_corr ev3_ev3_corr ev4_ev4_corr ev5_ev5_corr ev6_ev6_corr ev7_ev7_corr ev8_ev8_corr
     ...
     quad_quad_corr quad1_quad1_corr quad2_quad2_corr;
run;
data Exports.allcorr;
 set allcorr;
run;
%zexportcsv(allcorr,allcorr,&path1\analysis_sets\quad\exports);

data SASdata.allcorr;
 set allcorr;
run;
%zexportcsv(allcorr,allcorr,&path1\analysis_sets\quad\datasets);
