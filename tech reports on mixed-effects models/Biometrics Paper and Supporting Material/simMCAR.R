###########################################################################
### Simulated data on the CD4 cell count decline under MCAR missingness ###
### One can work with MCAR missingness to produce MAR data based on a   ###
### subject reaching a certain threshhold value of CD4-see SAS program  ###
###########################################################################
...
######################################
### Simulation of it MCAR datasets ###
######################################
...
for (j in 1:it)
{
  ...
  ######################################
  ### beta_w = -2.2808 gives a 40%   ###
  ### dropout over a 5 year time     ###
  ### span under a MCAR mechanism as ###
  ### S(5)=exp(-exp(-2.2808)*5)=.60  ###
  ######################################
  beta_w = c(-2.2808)
  kappa_w = 1.0
  ...
  for (i in 1:G)
  {
     surv[i] = rweibull(beta_w,kappa_w,x = cbind(1),time.axis_w)
  }
  ...
  # true survival time and 5 years (study termination) and threshold reached
