...
libname Results  "&path1\analysis_sets\acr12\correlations\results";
...
data t1;
 ...
 if compress(label) in ("LogHR(TrtID=2:TrtID=1)forevent1" 
						...
						"beta41att=12Months" "beta42att=12Months" "beta42-beta41att=12Months"
                        "beta41att=24Months" "beta42att=24Months" "beta42-beta41att=24Months"
						"beta41att=36Months" "beta42att=36Months" "beta42-beta41att=36Months"
						"beta41att=48Months" "beta42att=48Months" "beta42-beta41att=48Months"
						"beta31att=3Months" "beta32att=3Months" "beta32-beta31att=3Months"
                        "MeanBaselineGFR" 
                        "MeanLogACR" 
                        "TrtmenteffectindeltaGFRacute" 
                        "kappa" "beta12/beta11"
                        "TrtmenteffectinACRchange"
                        "ControlACRchange"
                        "TreatedACRchange");
 ...
 if compress(label) in ("beta42-beta41att=12Months")       then var="ts1y   ";
 if compress(label) in ("beta41att=12Months")              then var="ts1y1  ";
 if compress(label) in ("beta42att=12Months")              then var="ts1y2  ";

 if compress(label) in ("beta42-beta41att=24Months")       then var="ts2y   ";
 if compress(label) in ("beta41att=24Months")              then var="ts2y1  ";
 if compress(label) in ("beta42att=24Months")              then var="ts2y2  ";

 if compress(label) in ("beta42-beta41att=36Months")       then var="ts3y   ";
 if compress(label) in ("beta41att=36Months")              then var="ts3y1  ";
 if compress(label) in ("beta42att=36Months")              then var="ts3y2  ";

 if compress(label) in ("beta42-beta41att=48Months")       then var="ts4y   ";
 if compress(label) in ("beta41att=48Months")              then var="ts4y1  ";
 if compress(label) in ("beta42att=48Months")              then var="ts4y2  ";
 ...
 if compress(label) in ("ControlACRchange")                then var="dacr1  ";
 if compress(label) in ("TreatedACRchange")                then var="dacr2  ";
...
data allcorr;
 ...
 drop ev1_ev1_corr ev2_ev2_corr ev3_ev3_corr ev4_ev4_corr ev5_ev5_corr ev6_ev6_corr ev7_ev7_corr ev8_ev8_corr
     ...
     ts1y_ts1y_corr ts1y1_ts1y1_corr ts1y2_ts1y2_corr ts2y_ts2y_corr ts2y1_ts2y1_corr ts2y2_ts2y2_corr
     ts3y_ts3y_corr ts3y1_ts3y1_corr ts3y2_ts3y2_corr ts4y_ts4y_corr ts4y1_ts4y1_corr ts4y2_ts4y2_corr
     gfr_gfr_corr lacr_lacr_corr dgfr_dgfr_corr kappa_kappa_corr rchr_rchr_corr
     dacr_dacr_corr dacr1_dacr1_corr dacr2_dacr2_corr;
run;
data Exports.allcorr;
 set allcorr;
run;
%zexportcsv(allcorr,allcorr,&path1\analysis_sets\acr12\exports);

data SASdata.allcorr;
 set allcorr;
run;
%zexportcsv(allcorr,allcorr,&path1\analysis_sets\acr12\datasets);
