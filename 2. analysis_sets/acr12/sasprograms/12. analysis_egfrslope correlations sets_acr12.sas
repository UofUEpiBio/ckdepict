...
libname temp "&path1\analysis_sets\acr12\correlations\temp";
libname temp2 "&path1\analysis_sets\acr12\correlations\temp2";
...
run;
 ...
 delta_lg_ACR=logdelupro12;
 if logdelupro12=. then delete;
 keep allrx new_id lg_b_ACR delta_lg_ACR include_acr;
run;
proc means data=acr mean;
 var lg_b_ACR;
 class allrx;
 ods output summary=base_acr_centered(drop=nobs);
run;
proc sort data=base_acr_centered;
 by allrx;
run;
proc sort data=acr;
 by allrx;
run;
data acr;
 merge acr base_acr_centered;
 by allrx;
run;
data acr;
 set acr;
 lg_b_ACR=lg_b_ACR-lg_b_ACR_Mean;
 drop lg_b_ACR_Mean;
run;
/** Acute **/
data acute;
...
run;
data studies1;
 merge GFR(in=a) events(in=b) bacr acute acr;
 by allrx new_id;
run;
data studies1;
 set studies1;
...
%options_off;
%zInitialModelOptions(_POM_=SS,_ACR_=YES,_CE_=YES,_Prop_=YES,_BOUNDS_=NO,_SP_threshold=15,_PSI21_=YES, _HESS_=YES,_bGFR_=YES,
