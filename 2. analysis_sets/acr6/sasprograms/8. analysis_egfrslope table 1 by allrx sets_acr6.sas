...
proc sort data=t3;
 by datagrp2 zallrx;
run;
%zexportcsv(t3,&out1,&path1\analysis_sets\acr6\exports);
...
data updatedallrxdata;
 set t3;
run;
%zexportcsv(updatedallrxdata,updatedallrxdata,&path1\analysis_sets\acr6\datasets);

data Exports.updatedallrxdata;
 set t3;
run;
%zexportcsv(updatedallrxdata,updatedallrxdata,&path1\analysis_sets\acr6\exports);
...
proc sort data=t2;
 by datagrp2 intervention;
run;
%zexportcsv(t2,&out2,&path1\analysis_sets\acr6\exports);
proc datasets nolist;
...
proc sort data=t2;
 by datagrp2 disease;
run;
%zexportcsv(t2,&out3,&path1\analysis_sets\acr6\exports);
proc datasets nolist;
 delete t1 t2 xt1 t0 size;
run;
quit;
...
proc sort data=t2;
 by datagrp2 disease2;
run;
%zexportcsv(t2,&out4,&path1\analysis_sets\acr6\exports);
proc datasets nolist;
