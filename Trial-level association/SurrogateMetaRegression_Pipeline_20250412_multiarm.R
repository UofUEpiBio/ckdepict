...
# eventnumber - removed [2025/04/12]
...
# Default is no multi-arm trials. 
# *Update 23/11/02: revised handling of multi-arm corr so that the multi-arm trials 
#                   in analysis subset are assigned with respective corr;
#                   multi-arm trials are assumed to come in pairs (2), and one value
#                   in multi_arm_corrs corresponds to one such pair
...
trial_MR = function(data_location,data_file,output_location,model_number=1,
                    pooled="2",outcome_SE_cutoff=100000,exclude_interventions=c(""),exclude_trial_names=c(""),
  ...
  if(any(is.na(exclude_trial_names))) {
  ...
  myData=myData[(!myData$pooled %in% pooled)&
  ...
  if(length(which(myData$allrxname %in% multi_arm_trials)) > 0) {
    ...
    myData=rbind(d1,d2)
    # inds = match(multi_arm_trials,myData$allrxname)[!is.na(match(multi_arm_trials,myData$allrxname))]
    # multi_arm_corrs = unique(multi_arm_corrs[inds]) 
    # Update 23/11/02: 350-351 disabled as its function overlaps that of 388-391
  }
  ...
  if (length(independent_effects) == 1) {
    ...
    t_multiarm=data.frame(allrxname=multi_arm_trials,multi_arm_corrs=unlist(lapply(multi_arm_corrs,rep,2)))
    t_multiarm=subset(t_multiarm,allrxname %in% myData$allrxname[myData$allrxname %in% multi_arm_trials])
    data$nMultiArm<-nrow(t_multiarm)/2 #Number of pairs
    #Get correlation for each pair by extracting every 2nd rows
    multi_arm_corrs_index<-(1:nrow(t_multiarm))[(1:nrow(t_multiarm)) %% 2 == 0]
    data$multi_arm_corrs<-t_multiarm$multi_arm_corrs[multi_arm_corrs_index]
  }
  ...
  if (model_type == "random" & length(independent_effects) == 1) {
    ...
    stanmod=stan(model_code=myModel,data=data,iter=num.iter,chains=num_chains,
                 control=list(adapt_delta=0.99,max_treedepth=25),seed=123)
  ...
  model.info=c(paste("PS_data_",model_number,".csv",sep=""),
               as.character(data_file),as.character(pooled),as.character(outcome_SE_cutoff),
               ...
               as.character(substring(outcome_name_est,1,last=nchar(outcome_name_est)-4)),
               as.character(paste(substring(indep_var_name_est,1,last=nchar(indep_var_name_est)-4),collapse=",")),
               as.character(substring(cln_surr_corr_name,1,last=nchar(cln_surr_corr_name)-5)),
  ...
  colnames(model.info)=c("posteriors_saved_file","trial_data_used","pooled","outcome_SE_Cutoff",
                         "excluded_interventions","excluded_trial_names","subset condition 1","subset condition 2",
                         "subset condition 3","outcome_in_regression","independent_effects_in_regression","outcome_effect_corr",
