...
data quad;
 ...
 set gfrlopes_ma;
 parameter="Quadratic";
 estimate=beta21_beta22_est;
 se=beta21_beta22_se;
 estimate1=beta21_est;
 se1=beta21_se;
 estimate2=beta22_est;
 se2=beta22_se;
 parameternumber=2;
 keep study parameter parameternumber estimate se estimate1 se1 estimate2 se2 zallrx allrx allrxname allrxname1 study_code npatients nevents region region_fmt
      pooled pooled_fmt intervention intervention_fmt intervention_abrev disease disease_fmt disease2 disease2_fmt;
...
proc sort data=txt01;
 by study;
run;
data slopes_t99_quad_ma;
 merge slopes_t99_ma txt01;
 by study;
run;
proc sort data=slopes_t99_quad_ma;
 by parameternumber zallrx;
run;
data SASdata.slopes_t99_quad_ma;
 set slopes_t99_quad_ma;
run;
%zexportcsv(slopes_t99_quad_ma,slopes_t99_quad_ma,&path1\analysis_sets\quad\Datasets);

data Exports.slopes_t99_quad_ma;
 set slopes_t99_quad_ma;
run;
%zexportcsv(slopes_t99_quad_ma,slopes_t99_quad_ma,&path1\analysis_sets\quad\Exports);
